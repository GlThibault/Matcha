$("#slider-range-age").slider({
    ...
    max: 99,
    values: [18, 99],
    slide: function(event, ui) {$("#amount-age").val(ui.values[0] + " - " + ui.values[1])}
})
...
$("#slider-range-pop").slider({
    ...
    max: 30,
    values: [0, 30],
    slide: function(event, ui) {$("#amount-pop").val(ui.values[0] + " - " + ui.values[1])}
})
...
function ageAscending(a, b) {
    return $(a).find("#age").text() > $(b).find("#age").text() ? 1 : -1}
