...
router.get('/:username', function(req, res, next) {
    if (req.session && req.session.user == req.params.username) {
        ...
        connection.query('SELECT COUNT(*) AS count FROM block WHERE username = ? AND blocked = ?', [req.session.user, req.params.username], (err, result) => {
            ...
            if (result[0].count == 0) {
                connection.query('INSERT INTO block SET username = ?, blocked = ?', [req.session.user, req.params.username], (err, result) => {
                    if (err) console.log(err)
                    else
                        res.redirect('/')
                })
            } else {
                connection.query('DELETE FROM block WHERE username = ? AND blocked = ?', [req.session.user, req.params.username], (err, result) => {
                    if (err) console.log(err)
                    else
                        res.redirect('../u/' + req.params.username)
                })
